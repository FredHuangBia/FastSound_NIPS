Note that the Bullet Physics Engine is a modified version from its original repository.
The modified scripts are in ./modified_scripts/
They are linked to the corresponding CMake locations.

```
Usage: ./bin/App_RigidBodyFromObjExampleGui_gmake_x64_release bullet.cfg bullet.input.dat
Input: bullet.cfg -- GUI, fps, duration, distance, cameraSpecs (see main_opengl_single_example.cpp for format)
       bullet.input.dat -- objNum, objSpecs (see RigidBodyFromObj.cpp for format)
Output: collision_info.dat -- record collision information only
        motion_info.dat -- record movement of each object during simulation
```
