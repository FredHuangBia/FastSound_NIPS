...
def linear_random(min_val, max_val):
    ...
    return (max_val+min_val)/2 + (np.random.random(1)-0.5)*(max_val-min_val)
...
def map_to_prob(distance, time):
    t = float(time)
    return np.exp(-distance * (0.5+t*t/30))

def gaussian_random(max_val,min_val):
    return np.random.randn(1)*VAR*(max_val-min_val)/2
