...
def update_label(label, Id, attr=None):
    ...
    if Id == 4:
        newlabel[4] = 10**(gaussian_random(ALPHA_MAX, ALPHA_MIN)[0] + forward_mapping(label[4],Id))
    if Id == 5:
        newlabel[5] = 2**(gaussian_random(BETA_MAX, BETA_MIN)[0] + forward_mapping(label[5],Id))
    if Id == 6:
