...
function M.parse(arg)
  ...
  cmd:option('-GPUs',             '1,2',            'ID of GPUs to use by default, separated by ,')
  ...
  ------------- Data options ----------------------
  cmd:option('-nThreads',         40,              'number of data loading threads')
  cmd:option('-dataset',          'primV3b_5000',      'Options: primV2c, primV2e, primV3a(same as primV2d), primV3b')
  ...
  cmd:option('-epochNumber',      -1,             'Manual epoch number: 0=retrain|-1=latest|-2=best')
  ...
  cmd:option('-netType',          'cnnF',         'Options: cnnA(for primV2c), cnnB(for primV2d and primV3a), cnnE(for primV2e), cnnF(for primV3b)')
  cmd:option('-netSpec',          'soundnet8',    'Options: custom|res{18|34|50|101|200}|soundnet{5|8}')
...
function M.init(opt)
 ...
 -- dataset-specific configurations
  if opt.dataset == 'primV2c' then
    ...
    opt.numEntry = 9072
    opt.numObj = 1
  elseif opt.dataset == 'primV2d' or opt.dataset == 'primV3a'then
    opt.audioRate = 44100
    opt.audioLen = 3
    opt.audioDim = opt.audioLen * opt.audioRate
    opt.maxObj = 1
    opt.maxXmlLen = 6  -- 5 labels and 1 sound id
    opt.numObjId = 1
    opt.numEntry = 100000
    opt.numObj = 1    
  elseif opt.dataset == 'primV2e' then
    opt.audioRate = 44100
    opt.audioLen = 3
    opt.audioDim = opt.audioLen * opt.audioRate
    opt.maxObj = 1
  ...
  elseif opt.dataset == 'primV3b' then
    ...
    opt.numEntry = 200000
    opt.numObj = 1  
  elseif opt.dataset == 'primV3b_5000' then
    opt.audioRate = 44100
    opt.audioLen = 3
    opt.audioDim = opt.audioLen * opt.audioRate
    opt.maxObj = 1
  ...
  elseif opt.netType == 'cnnB' then
    opt.outputSize = 64+4
    opt.outputSplitType = torch.ones(opt.maxXmlLen)   -- 1 categorial, 2 normal, 3 bernoulli
    opt.criterionWeights = torch.ones(opt.maxXmlLen)
  elseif opt.netType == 'cnnE' then
    opt.outputSize = 100+10+64+4
    opt.outputSplitType = torch.ones(opt.maxXmlLen)   -- 1 categorial, 2 normal, 3 bernoulli
    opt.criterionWeights = torch.ones(opt.maxXmlLen)    
  elseif opt.netType == 'cnnF' then
    opt.outputSize = 14+10+64+4
    opt.outputSplitType = torch.ones(opt.maxXmlLen)   -- 1 categorial, 2 normal, 3 bernoulli
    opt.criterionWeights = torch.ones(opt.maxXmlLen)  
  elseif opt.netType == 'cnnES' then
    opt.outputSize = 100
    opt.outputSplitType = torch.ones(opt.maxXmlLen)   -- 1 categorial, 2 normal, 3 bernoulli
    opt.criterionWeights = torch.ones(opt.maxXmlLen)    
    -- opt.mseLoss = 0
