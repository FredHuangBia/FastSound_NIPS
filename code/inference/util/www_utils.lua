...
function gen_sound(nn_output, path, wavName , dataset, target_xml )
  ...
  elseif dataset == 'primV2d' or dataset == 'primV3a'then  -- rotation height alpha beta restitution    
    -- file = io.open (paths.concat(path,'../','output_value.txt'), 'a+')
  --   file:write( infer_values[1] .. ' ' .. infer_values[2] .. ' ' .. infer_values[3] .. ' ' .. infer_values[4] .. ' ' .. infer_values[5] ..'\n')
    -- file:close()    
...
function genInferenceAudio(audioDir, captions, audio2Names , dataset) 
  ...
  for i = 1, (#captions) do
    ...
    elseif dataset == 'primV2e' or dataset=='primV3b' or dataset=='primV3b_5000' then
      ...
      -- file = io.open (paths.concat(audioDir,'../','output_xml.txt'), 'a+')
      -- file:write( output_xml[1] .. ' ' .. output_xml[2] .. ' ' .. output_xml[3] .. ' ' .. output_xml[4] .. ' ' .. output_xml[5] ..' ' .. output_xml[6]' ' .. output_xml[7]..'\n')
      -- file:close()
      -- file = io.open (paths.concat(audioDir,'../','target_xml.txt'), 'a+')
      -- file:write( target_xml[1] .. ' ' .. target_xml[2] .. ' ' .. target_xml[3] .. ' ' .. target_xml[4] .. ' ' .. target_xml[5] ..' ' .. target_xml[6]' ' .. target_xml[7]..'\n')
      -- file:close()
