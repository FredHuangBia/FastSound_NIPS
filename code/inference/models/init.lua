...
function M.setup(opt, checkpoint, dataLoader)
  local model
  ...
  if checkpoint then
    ...
    if not paths.filep(modelPath) then 
      print('=> WARNING: Saved model not found: ' .. modelPath)
    else
  ...
  if not model then
    ...
    model = require('models/' .. opt.netType)(opt, dataLoader)
  end
  ...
  -- load optimState checkpoint
  local optimState 

  if checkpoint then
    local optimPath = paths.concat(opt.resume, checkpoint.optimFile)
    if not paths.filep(optimPath) then
      print('=> WARNING: Saved optimState not found: ' .. optimPath)
    else
