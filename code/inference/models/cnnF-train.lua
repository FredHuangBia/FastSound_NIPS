...
local M = {}
local Trainer = torch.class('cnnF.Trainer', M)
...
function Trainer:train(epoch, dataloader)
  ...
  for n, sample in dataloader:run() do  -- a sample is a batch
    ...
    self.model.output[{{},{1,14}}] = self.model.output[{{},{1,14}}]*weights[1]
    self.target[{{},{1,14}}] = self.target[{{},{1,14}}]*weights[1]

    self.model.output[{{},{15,24}}] = self.model.output[{{},{15,24}}]*weights[2]
    self.target[{{},{15,24}}] = self.target[{{},{15,24}}]*weights[2]

    self.model.output[{{},{25,88}}] = self.model.output[{{},{25,88}}]*weights[3]
    self.target[{{},{25,88}}] = self.target[{{},{25,88}}]*weights[3]

    self.model.output[{{},{89,92}}] = self.model.output[{{},{89,92}}]*weights[4]
    self.target[{{},{89,92}}] = self.target[{{},{89,92}}]*weights[4]
