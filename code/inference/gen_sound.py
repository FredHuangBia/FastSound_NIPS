...
ROOT_DIR = '/data/vision/billf/object-properties/sound/sound/primitives/exp/single_shape/'
TARGET_DIR = argv[1]
...
with cd(simFilePath):
    if skip_bullet!=True:
        ...
        subprocess.call('sh %s >log1.txt'%os.path.join(ROOT,'scripts','prepare_dat.sh'),shell=True)
    ...
    if (skip_sound!=True):
        for obj_id in range(0,objnum):
            ...
            print 'sh %s'%os.path.join(ROOT,'scripts','prepare_ini.sh')+' -i %d '%obj_id\
            ...
            if not os.path.exists('./../sound.wav') or not os.path.exists('./../sound.raw'):
                ...
                # subprocess.call('rm *.wav',shell = True)
                # subprocess.call('rm *.raw',shell = True)
                subprocess.call('bash %s'%os.path.join(ROOT,'scripts','prepare_ini.sh')+' -i %d '%obj_id\
