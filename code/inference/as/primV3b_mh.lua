...
function gen_sound(nn_output, path, id, i, j, k)
	-- synthesis new sound based on given params
	local real_values = label2Val(nn_output)
	local outputDir = path .. ('%06d/'):format(id) .. ("%d-%d-%d/"):format(i,j,k)
...
function log_likelihood(fv1, fv2)
	...
	print(torch.dist(fv1,fv2,2)*20)
	return math.exp(-torch.dist(fv1,fv2,2)*10)
end
...
function a_b_s(model, id, iterations, numsample, verbose)
	-- pass in an initialization table of 7 params
	local path = '/data/vision/billf/object-properties/sound/sound/primitives/exp/temp/'
	...
	local init_sound = gen_sound(params, path, id, 0, 0, 0)
	...
	local best_i, best_j, best_k = 0, 0, 0
	for i = 1, iterations do
		...
		for j = 1, #params do
			...
			for k = 1, numsample do
				...
				local new_sound = gen_sound(new_params, path, id, i, j, k)
		...
		dist_log:write(("%d %.4f %d-%d-%d | %d %d %d %f %f %f %f | %d %d [%f,%f,%f,%f] %f %f %f %f\n"):format(i,ll, best_i,best_j,best_k, params[1],params[2],params[3],params[4],params[5],params[6],params[7], best_real[1], modulus_mapping[params[2]], best_real[3][1],best_real[3][2],best_real[3][3],best_real[3][4],best_real[4],best_real[5],best_real[6],best_real[7]))
	end
	print("=============================Final Result=============================")
	...
	print("Estimation:")
	print(params)
	local best_dir = path .. ('%06d/'):format(id) .. ("%d-%d-%d/"):format(best_i,best_j,best_k)
...
num_sampling = tonumber(arg[3])
a_b_s(pretrainedModel,id,num_iteration,num_sampling,true)
