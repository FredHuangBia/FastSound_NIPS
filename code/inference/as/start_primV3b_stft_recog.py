...
path = '/data/vision/billf/object-properties/sound/sound/primitives/exp/primV3b_stft_recog/'
...
for target in targets[6*split:6*(split+1)]:
	if not target in finished:
		cmd = 'CUDA_VISIBLE_DEVICES='+str(GPU)+' th '+' primV3b_synthesis_stft_recog.lua '+str(target)+' 30'
		print(cmd)
