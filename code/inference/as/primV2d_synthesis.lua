...
function gen_sound(nn_output, path, id, i, j)
	...
	os.execute("python " .. scripts .. "gen_sound.py " .. outputDir .. " 0 0 100000 10 10 10 0 0 0 0 > " .. outputDir .. 'gen_sound.log')
	return outputDir
end
...
function a_b_s(model, id, iterations, verbose)
	...
	for i = 1, iterations do
		...
		dist_log:write(("%d %.4f %d-%d %d %.2f %.2f %.2f %.2f [%.2f,%.2f,%.2f,%.2f] %.2f %f %.2f %.2f\n"):format(i,ll, best_i,best_j, params[1],params[2],params[3],params[4],params[5], best_real[1][1],best_real[1][2],best_real[1][3],best_real[1][4],best_real[2],best_real[3],best_real[4],best_real[5]))
	end
	print("=============================Final Result=============================")
