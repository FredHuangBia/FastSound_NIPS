...
function gen_sound(nn_output, path, id, i, j)
	...
	os.execute("python " .. sound_scripts .. "gen_sound.py " .. outputDir .. " 0 0 101 10 10 10 " .. real_values[1] .. real_values[2] .. " 0 0 0 > " .. outputDir .. 'gen_sound.log')
	return outputDir
end
...
function a_b_s(model, id, iterations, verbose)
	...
	local path = '/data/vision/billf/object-properties/sound/sound/primitives/exp/primV2e/'
	local all_sound = '/data/vision/billf/object-properties/sound/sound/primitives/data/primV2e/'
	...
	local shape = torch.Tensor(1,100)
	local modulus = torch.Tensor(1,10)
	local rotation = torch.Tensor(1,64)
	shape[1] = init[1][{{1,100}}]:double()
	modulus[1] = init[1][{{101,110}}]:double()
	rotation[1] = init[1][{{111,174}}]:double()
...
pretrainedModel = '/data/vision/billf/object-properties/sound/sound/primitives/models/primV2e_cnnE_soundnet8_pretrainnone_mse1_LR0.001/model_130.t7'
id = tonumber(arg[1])
