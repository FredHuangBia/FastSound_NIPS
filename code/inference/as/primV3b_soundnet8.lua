-- usage: CUDA_VISIBLE_DEVICES=GPUID th primV3b_soundnet8.lua shapeID iteration rand(1 for rand,0 for prior)
...
soundnet_8_path = '/data/vision/billf/object-properties/lib/caffe/models/soundnet_8/soundnet8_final.t7'
pretrainedModel = '/data/vision/billf/object-properties/sound/sound/primitives/models/primV3b_cnnF_soundnet8_pretrainnone_mse1_LR0.001/model_150.t7'
...
function sample_discrete_rand(num)
...
function a_b_s(model, id, iterations, verbose, random)
	...
	local soundnet_8 = torch.load(soundnet_8_path)
	for i = 1,4 do
	soundnet_8:remove()
	end

	print("==> Generating Initialization!")
	local initialization={}
	if random then
		print('Random init.')
		initialization = random_init()
	else 
		print('None random init.')
		local soundnet = load_model(model)
		soundnet:evaluate()
		init = soundnet:forward(real_sound_t7)
		...
		shape_cdf = get_distribution(shape)
		modulus_cdf = get_distribution(modulus)
		rot_cdf = get_distribution(rotation)
	...
	-- end
	-- print(weights)
	local target_feature = get_feature(real_sound_t7, soundnet_8)
	...
	local init_feature = get_feature(init_sound_t7,soundnet_8)
...
random = tonumber(arg[3])
a_b_s(pretrainedModel,id,num_iteration,true,random==1)
