...
function a_b_s(model, id, iterations, verbose)
	...
	local path = '/data/vision/billf/object-properties/sound/sound/primitives/exp/primV3b_corl_recog/'
	...
	local real_sound_raw = read_raw(real_sound)
	local autocorrelation = get_correlation(real_sound_raw,real_sound_raw)
	...
	local init_sound_raw = read_raw(init_sound)
	-- local init_feature = get_feature(init_sound_t7,soundnet, weights)
	local crosscorrelation = get_correlation(real_sound_raw,init_sound_raw)
	local ll = log_likelihood(autocorrelation, crosscorrelation)
	...
	for i = 1, iterations do
		...
		for j = 1, #params do
			...
			local new_sound_raw = read_raw(new_sound)
			-- local new_feature = get_feature(new_sound_t7, soundnet, weights)
			local new_correlation = get_correlation(real_sound_raw,new_sound_raw)
			local new_ll = log_likelihood(autocorrelation,new_correlation)
			if new_ll < ll then
