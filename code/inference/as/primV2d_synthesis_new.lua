...
function a_b_s(model, id, iterations, verbose)
	...
	local path = '/data/vision/billf/object-properties/sound/sound/primitives/exp/primV2d/193/'
	...
	dist_log = io.open(path .. ('%06d/'):format(id) .. "dist.log", "w")
	...
	local sigma = {1/3,1/4,1/6,1/3,1/3}
	...
	local raw_out = io.open('/data/vision/billf/object-properties/sound/sound/primitives/target.txt','w')
	for k = 1, (#target_feature)[2] do
		raw_out:write(target_feature[1][k]..'\n')
	end

	print("==> Sampling starts!")
	local best_i, best_r = 0, 0
	local order = {5,5,4,4,5,5,4,4,2,2,3,1}
	for i = 1, iterations do
		print(("=============================iteration %d============================="):format(i))
		for r,j in pairs(order) do
			...
			local new_sound = gen_sound(new_params, path, id, i, r)
			...
			if new_ll < ll then
				local raw_out = io.open('/data/vision/billf/object-properties/sound/sound/primitives/'..i..'-'..r..'.txt','w')
				for k = 1, (#target_feature)[2] do
					raw_out:write(new_feature[1][k]..'\n')
				end
		...
		dist_log:write(("%d %.4f %d-%d %d %.2f %.2f %.2f %.2f [%.2f,%.2f,%.2f,%.2f] %.2f %.2f %.2f %.2f\n"):format(i,ll, best_i,best_r, params[1],params[2],params[3],params[4],params[5], best_real[1][1],best_real[1][2],best_real[1][3],best_real[1][4],best_real[2],best_real[3],best_real[4],best_real[5]))
	end
	print("=============================Final Result=============================")
	...
	print("Estimation:")
	print(params)
	local best_dir = path .. ('%06d/'):format(id) .. ("%d-%d/"):format(best_i,best_r)
	print("Output directory for the best estimate: " .. best_dir)
	local best = io.open(path .. ('%06d/'):format(id) .. "best.txt", "w")
	best:write(("%d-%d %d %.2f %.2f %.2f %.2f\n"):format(best_i,best_r, params[1],params[2],params[3],params[4],params[5]))
	best:write(("[%.2f,%.2f,%.2f,%.2f] %.2f %.2f %.2f %.2f\n"):format(best_real[1][1],best_real[1][2],best_real[1][3],best_real[1][4],best_real[2],best_real[3],best_real[4],best_real[5]))
end

pretrainedModel = '/data/vision/billf/object-properties/sound/sound/primitives/models/primV2d_cnnB_soundnet8_pretrainnone_mse1_LR0.001/model_193.t7'
id = tonumber(arg[1])
