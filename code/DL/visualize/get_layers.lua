...
model = '/data/vision/billf/object-properties/sound/sound/primitives/models/primV4a_cnnCor2_soundnet8_pretrainnone_mse1_LR0.001/model_best.t7'
...
-- for i=1,16 do
-- 	filter_out = io.open('/data/vision/billf/object-properties/sound/sound/primitives/code/inference/visualize/oracle/filters/filter'..i..'.txt','w')
-- 	for j = 1,64 do
...
local activations = torch.zeros(test_num,1024)
activations = activations:cuda()
local test_path = '/data/vision/billf/object-properties/sound/sound/primitives/data/primV4a/'
for i = 1,test_num do
    local outer_folter = ('%.03d'):format( math.floor((i)/100) +1)
    local str = string.format("%06d", i)
    local t7_path = path.join(test_path,outer_folter,str,'merged.t7')
...
last_layer_fout = io.open('/data/vision/billf/object-properties/sound/sound/primitives/code/inference/visualize/weak/activation_last_layer.txt','w')
for i = 1,1024 do
	local top5 = torch.zeros(5)
	local activation = torch.zeros(test_num,1):cuda()
	local maximum = -100
	activation[{{1,test_num},{1,1}}] = activations[{{1,test_num},{i,i}}]
	for j = 1,5 do
