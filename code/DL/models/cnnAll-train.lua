...
local visepoch = 1
local vissound = 200
local include_rot = 0 -- rotation, 3,4,5,6

local M = {}
local Trainer = torch.class('cnnAll.Trainer', M)
...
function Trainer:train(epoch, dataloader)
  ...
  for n, sample in dataloader:run() do  -- a sample is a batch
    ...
    self.model.output[{{},{3,6}}] = self.model.output[{{},{3,6}}]*include_rot
    self.target[{{},{3,6}}] = self.target[{{},{3,6}}]*include_rot
