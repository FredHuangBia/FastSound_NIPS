...
local function findAudios(opt)
  ...
  for i = 1, numData do
    ...
    -- '1-6', '2-7', '3-8', '4-13', '5-14'
    
    xml[i][1] = pieces[2]+1 -- label 1 (shape)
    xml[i][2] = pieces[3]+1 -- label 2 (height)
    xml[i][3] = pieces[4]   -- label 3 (alpha)
    xml[i][4] = pieces[5]   -- label 4 (restitution)
    xml[i][5] = pieces[1]   --  sound id
    xmlLen[i] = 8 -- current length of xml

    local str = string.format("%06d", tonumber(pieces[1]))

    local outer_folter = ('%.03d'):format( math.floor((tonumber(pieces[1])-0.5)/1000) +1)
