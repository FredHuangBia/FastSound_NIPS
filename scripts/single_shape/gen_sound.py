...
ROOT_DIR = '/data/vision/billf/object-properties/sound/sound/primitives/'
...
class Obj:
    ...
    def ReadPose(self,poseId):
        ...
        cfgPath = os.path.join(self.ROOT,'config/single_shape','pose','pose-%d-%d.cfg'%(self.poseId[0],self.poseId[1]))
    ...
    def ReadBulletSetup(self,bulletSetupId):
        ...
        cfgPath = os.path.join(self.ROOT,'config/single_shape','bullet_setup','bullet_%d.cfg'%self.bulletSetupId)
    ...
    def ReadMaterial(self,matId):
        ...
        cfgPath = os.path.join(self.ROOT,'config/single_shape','material','material-%d-%d-%d.cfg'%(self.matId[0],self.matId[1],self.matId[2]))
        print cfgPath
        matCfg.read(cfgPath)
    ...
    def ReadObj(self,objId):
        ...
        #    self.mass = 300
        if self.active == 1:
            self.objPath = os.path.join(self.ROOT,'data/single_shape','%d'%self.objId,'%d.orig.obj'%self.objId)
        else:
            self.objPath = os.path.join(self.ROOT,'data/single_shape','%d'%self.objId,'%d.orig.obj'%self.objId)
        if self.active == 1:
            volumefile = open(os.path.join(self.ROOT,'data/single_shape/%d/volume.txt'%self.objId))
            self.mass = float(volumefile.readline())*self.density/5000
            print self.mass
        # Load mass
    ...
    def WriteShellCmd(self):
        cmd = '-n %d -d %f -A %1.2E -B %f'%(self.objId,self.density,Decimal(self.alpha),self.beta)
        return cmd
...
class Cam:
    ...
    def __init__(self,cfgId=0):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config/single_shape','camera','camera_%d.cfg'%cfgId)
        self.Load()
...
class Lighting:
    ...
    def __init__(self,cfgId=0):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config/single_shape','lighting','lighting_%d.cfg'%cfgId)
    def SetCfgId(self,cfgId):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config/single_shape','lighting','lighting_%d.cfg'%cfgId)
    def PrintPath(self):
...
if __name__ == '__main__':
    ...
    print argv[1]
    sceneid = (int(argv[1]),int(argv[2]));
    sceneConfig = ConfigParser.ConfigParser();
    sceneConfig.read(os.path.join(ROOT_DIR,'config/single_shape/scene/scene-%d-%d.cfg'%(sceneid[0],sceneid[1])))
    
    # Get Objs
    objnum = sceneConfig.getint('Objects','obj_num')
    if(len(argv)<(3+4*objnum)):
        print 'not enough arguemnt! using default objects and materials\n'
        objs = [Obj(0,0) for x in range(objnum)]
    else:
        objs = []
        for k in range(objnum):
            objs.append(Obj(objId=int(argv[k*4+3]),matId=(int(argv[k*4+4]),int(argv[k*4+5]),int(argv[k*4+6]))))
    ...
    simCfg.read(os.path.join(ROOT,'config/single_shape','simulation','sim_%d.dat'%simId))
    ...
    resultPath = os.path.join(ROOT,'result/single_shape/v1','scene-%d-%d'%(sceneid[0],sceneid[1]),matResultPath)
    ...
    with cd(simFilePath):
        if skip_bullet!=True:
            for obj in objs:
                ...
                if not os.path.exists('%d.obj'%obj.objId):
                    subprocess.call('ln -s %s'%os.path.join(ROOT,'data/single_shape','%d'%obj.objId,'%d.obj'%obj.objId)\
                                +' %d.obj'%obj.objId,shell=True)
                if not os.path.exists('%d.orig.obj'%obj.objId):
                    subprocess.call('ln -s %s'%os.path.join(ROOT,'data/single_shape','%d'%obj.objId,'%d.orig.obj'%obj.objId)\
            ...
            subprocess.call('sh %s >log1.txt'%os.path.join(ROOT,'scripts/single_shape','prepare_dat.sh'),shell=True)
        ...
        if (skip_sound!=True):
            for obj_id in range(0,objnum):
                ...
                subprocess.call('ln -s %s'%os.path.join(ROOT,'data/single_shape','%d'%objs[obj_id].objId,\
                ...
                print 'sh %s'%os.path.join(ROOT,'scripts/single_shape','prepare_ini.sh')+' -i %d '%obj_id\
                ...
                if not os.path.exists('./../sound.wav') or not os.path.exists('./../sound.raw'):
                    ...
                    subprocess.call('rm *.wav',shell = True)
                    subprocess.call('rm *.raw',shell = True)
                    subprocess.call('bash %s'%os.path.join(ROOT,'scripts/single_shape','prepare_ini.sh')+' -i %d '%obj_id\
        ...
        if skip_rendering!=True and (not os.path.exists('./../sli.mp4') or is_overwrite):
            ...
            blank = os.path.join(ROOT,'scripts/single_shape','blank.blend')
            blenderScript = os.path.join(ROOT,'scripts/single_shape','blender_render_scene.py')
