...
class Obj:
    ...
    def ReadPose(self,poseId):
        ...
        cfgPath = os.path.join(self.ROOT,'config/test','pose','pose-%d-%d.cfg'%(self.poseId[0],self.poseId[1]))
    ...
    def ReadBulletSetup(self,bulletSetupId):
        ...
        cfgPath = os.path.join(self.ROOT,'config/test','bullet_setup','bullet_%d.cfg'%self.bulletSetupId)
    ...
    def ReadMaterial(self,matId):
        ...
        cfgPath = os.path.join(self.ROOT,'config/test','material','material-%d-%d-%d.cfg'%(self.matId[0],self.matId[1],self.matId[2]))
        print cfgPath
        matCfg.read(cfgPath)
    ...
    def ReadObj(self,objId):
        ...
        if self.active == 1:
            ...
            self.objPath = os.path.join(self.ROOT,'data/single_shape','%d'%self.objId,'%d.orig.obj'%self.objId)
        if self.active == 1:
            volumefile = open(os.path.join(self.ROOT,'data/single_shape/%d/volume_test.txt'%self.objId))
...
class Cam:
    ...
    def __init__(self,cfgId=0):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config/test','camera','camera_%d.cfg'%cfgId)
        self.Load()
...
class Lighting:
    ...
    def __init__(self,cfgId=0):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config/test','lighting','lighting_%d.cfg'%cfgId)
    def SetCfgId(self,cfgId):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config/test','lighting','lighting_%d.cfg'%cfgId)
    def PrintPath(self):
...
if __name__ == '__main__':
    ...
    sceneConfig.read(os.path.join(ROOT_DIR,'config/test/scene/scene-%d-%d.cfg'%(sceneid[0],sceneid[1])))
    ...
    simCfg.read(os.path.join(ROOT,'config/test','simulation','sim_%d.dat'%simId))
    ...
    resultPath = os.path.join(ROOT,'result/single_shape','scene-%d-%d'%(sceneid[0],sceneid[1]),matResultPath)
