...
SCRIPT_DIR = '/data/vision/billf/object-properties/sound/sound/primitives/scripts/multi_shape_v4/'
TARGET_DIR = argv[1]
...
class Obj:
    ...
    def ReadObj(self,objId):
        self.objId = objId
        if self.active == 1:
            self.objPath = os.path.join(self.ROOT,'%03d'%self.objId,'%03d.orig.obj'%self.objId)
        else:
            self.objPath = os.path.join(self.ROOT,'%03d'%self.objId,'%03d.orig.obj'%self.objId)
        if self.active == 1:
            volumefile = open(os.path.join(self.ROOT,'%03d/volume.txt'%self.objId))
            self.mass = float(volumefile.readline())*self.density/1000
            print self.mass
    ...
    def WriteShellCmd(self):
        cmd = '-n %03d -d %f -A %.3E -B %f'%(self.objId,self.density,Decimal(self.alpha),self.beta)
        return cmd
...
for obj_id in range(0,objnum):
    bltInputCfg.write('%d %03d '%(obj_id,objs[obj_id].objId) + objs[obj_id].WriteString())
...
with cd(simFilePath):
    if skip_bullet!=True:
        for obj in objs:
            ...
            if not os.path.exists('%03d.obj'%obj.objId):
                subprocess.call('ln -s %s'%os.path.join(ROOT,'%03d'%obj.objId,'%03d.obj'%obj.objId)\
                            +' %03d.obj'%obj.objId,shell=True)
    ...
    if (skip_sound!=True):
        for obj_id in range(0,objnum):
            ...
            if os.path.exists('%03d.vmap'%objs[obj_id].objId):
                subprocess.call('unlink %03d.vmap'%objs[obj_id].objId,shell=True)
            subprocess.call('ln -s %s'%os.path.join(ROOT,'%03d'%objs[obj_id].objId,\
                            '%03d.vmap'%objs[obj_id].objId)\
                            +' %03d.vmap'%objs[obj_id].objId,shell=True)
            ...
            if os.path.exists('%03d.geo.txt'%objs[obj_id].objId):
                subprocess.call('unlink %03d.geo.txt'%objs[obj_id].objId,shell=True)  
            subprocess.call('ln -s %03s'%os.path.join(ROOT,'%d'%objs[obj_id].objId,\
                          '%03d.geo.txt'%objs[obj_id].objId)+\
                            ' %03d.geo.txt'%objs[obj_id].objId,shell=True)
            
            if os.path.exists('%03d.ev'%objs[obj_id].objId):
