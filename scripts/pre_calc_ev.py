...
class Obj:
    ...
    def ReadObj(self,objId):
        self.objId = objId
        # self.objPath = os.path.join(self.ROOT,'data/v1','%d'%self.objId,'models','%d.orig.obj'%self.objId)
    
    def ReadTet(self):
        self.tetPath = os.path.join(self.ROOT,'data/v1','%d'%self.objId,'%d.tet'%self.objId)
...
if __name__=='__main__':
    ...
    # objfilePath = os.path.join(ROOT_DIR,'data','v1','%d'%obj.objId)
    outPath = os.path.join(ROOT_DIR,'data','v1','%d'%obj.objId,'mat-%d'%obj.matId)
    CreateDir(outPath)
    renew = os.path.join('~','.renew')
    CreateDir(os.path.join(SOURCECODE,'www','EV_primitives_status',host))
    logfile = os.path.join(SOURCECODE,'www','EV_primitives_status',host,'%d-%d.txt'%(obj.objId,obj.matId))
    subprocess.call('echo start > %s'%logfile,shell=True)
    #call extmat, save to root
    with cd(outPath):
        if not os.path.exists('%d.tet'%obj.objId) or overwrite==1 :
            #print not os.path.exists('obj-%d.tet'%obj.objId)
            #print overwrite==1
            #subprocess.call('unlink obj-%d.tet'%(obj.objId),shell=True)
            subprocess.call('ln -s ../%d.tet %d.tet'%(obj.objId,obj.objId),shell=True)
            
        if not os.path.exists('%d.stiff.spm'%obj.objId) or not os.path.exists('%d.mass.spm'%obj.objId) or overwrite==1:
            cmd = EXTMAT + ' -f %d -y %.4g -p %.5g -m -k -g -s -d 1 | tee -a %s'%(obj.objId,obj.youngsModulus,obj.poissonRatio,logfile);
            subprocess.call(cmd ,shell=True)
            
    #call ev calculation, save to mat-id
        print('EV!')
        if not os.path.exists('%d.ev'%obj.objId) or overwrite==1:
            subprocess.call(renew ,shell=True)
            cmd = 'matlab -nodisplay -nodesktop -nosplash -r "addpath(\'%s\'); ev_generator60(\'%s\', 60); quit"| tee -a %s'%(FILEGENERATORS,'%d'%obj.objId,logfile)
            subprocess.call(cmd ,shell=True)
    
    #Geo maps
        if not os.path.exists('%d.vmap'%obj.objId) or overwrite==1:
            cmd = '%s/vmap_generator %d.geo.txt %d.vmap | tee -a %s'%(FILEGENERATORS,obj.objId,obj.objId,logfile)
            print cmd
            subprocess.call(cmd ,shell=True)
        ...
        if not os.path.exists('./bem_input/init_bem.mat') or \
            ...
            subprocess.call(renew ,shell=True)
            cmd = 'matlab -nodisplay -nodesktop -nosplash -r "addpath(\'%s\');BEMInputGenerator(\'%s\', \'%d\', %.5g, %.5g, %.5g,%d); quit" | tee -a %s'\
            %(FILEGENERATORS,outPath,obj.objId,obj.density,obj.alpha,obj.beta,overwrite,logfile)
            subprocess.call(cmd ,shell=True)

    
    CreateDir(os.path.join(SOURCECODE,'www','EV_primitives_status','%d-%d'%(obj.objId,obj.matId)))
