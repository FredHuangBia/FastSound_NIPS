...
ROOT = '/data/vision/billf/object-properties/sound/sound/primitives/data/primV3a/'
...
def get_label(v_range, value):		#!!!!!!!!!!!!!BUG!!!!!!!!!!!!
	mean = sum(v_range)/2
...
def main():
	...
	data_label = open(ROOT+'../labels3a/%06d-%06d.txt' %(start, end),'w')
	data_value = open(ROOT+'../labels3a/%06d-%06d.dat' %(start, end),'w')
	for i in range(start, end+1):
		...
		cmd = 'python ' + scripts + 'gen_sound.py ' + output_dir + ' 0 0 101 10 10 10 0 0 0 0 > log.txt'
		print(cmd)
		subprocess.call('kinit -R', shell=True)
		subprocess.call(cmd, shell=True)
		data_label.write("%06d %d %.4f %.4f %.4f %.4f\n" %(i, rotation_label, get_label(height_range, height), get_label(alpha_range, alpha), get_label(beta_range, beta), get_label(restitution_range, restitution)))
		data_value.write("%06d [%.3f,%.3f,%.3f,%.3f] %.3f %.3e %.3f %.3f\n" %(i, rotation[0], rotation[1], rotation[2], rotation[3], height, Decimal(math.pow(10,alpha)), math.pow(2,beta), restitution))
		data_label.flush()
		data_value.flush()
